...
def fill_matrix(k, keys, r, rows, arr, graph, added_numbers):
    for c in graph[keys[k]][rows[r]]:
        if arr[rows[r]][c] > 0:
            continue
        arr[rows[r]][c] = keys[k]
        added_numbers.append((keys[k], rows[r], c))  # Track the added number and its position
        if is_safe(rows[r], c, arr):
            if r < len(rows) - 1:
                if fill_matrix(k, keys, r + 1, rows, arr, graph, added_numbers):
                    return True
                else:
                    arr[rows[r]][c] = 0
                ...
                if k < len(keys) - 1:
                    if fill_matrix(k + 1, keys, 0, list(graph[keys[k + 1]].keys()), arr, graph, added_numbers):
                        return True
                    else:
                        arr[rows[r]][c] = 0
                        added_numbers.pop()  # Undo the change
...
def build_pos_and_rem(arr, pos, rem):
    for i in range(0, 9):
        for j in range(0, 9):
            if arr[i][j] > 0:
...
def main(arr):
    ...
    pos, rem = build_pos_and_rem(arr, pos, rem)
    rem = {k: v for k, v in sorted(rem.items(), key=lambda item: item[1])}
    build_graph(arr, pos, graph)

    # Fill the matrix
    key_s = list(rem.keys())
    fill_matrix(0, key_s, 0, list(graph[key_s[0]].keys()), arr, graph, added_numbers)
